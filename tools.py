...
def collision(robot, q):
    ...
    #              print("collision pair:",robot.collision_model.geometryObjects[cp.first].name,",",robot.collision_model.geometryObjects[cp.second].name,"- collision:","Yes" if cr.isCollision() else "No")
     ...
     #for k in range(len(robot.collision_model.collisionPairs)):
     #   if pin.computeCollision(robot.collision_model, robot.collision_data, k):
     #       print("collision pair:", robot.collision_model.geometryObjects[robot.collision_model.collisionPairs[k].first].name,",", robot.collision_model.geometryObjects[robot.collision_model.collisionPairs[k].second].name)
     
     return pin.computeCollisions(robot.collision_model,robot.collision_data,False) 
    
def cube_collision(cube):
    """Return true if cube in collision, false otherwise"""
...
def getcubeplacement(cube, hookname = None):
    ...
    if hookname is not None:
        ...
        if hookname == LEFT_HOOK:
            translation[1] = 0.002 
        elif hookname == RIGHT_HOOK:
            translation[1] = -0.002 
        trans_matrix = pin.SE3(np.eye(3), translation)  # Create a transformation matrix
        oMf = trans_matrix * oMf  # Apply the translation
    return oMf
